...
interface RefetchStatus {
  running: boolean;
  current: RefetchProgress | null;
  history: Record<string, RefetchHistoryEntry>;
}

async function fetchRefetchStatus(): Promise<RefetchStatus | null> {
  try {
...
async function triggerRefetch(source: string | null, limit: number): Promise<{ success: boolean; message?: string }> {
  try {
...
function RefetchButton({
  source, refetchable, disabled, onTriggered, onError,
}: {
  source: string | null;
  refetchable: number;
  disabled: boolean;
  ...
  const handleClick = async () => {
    setPending(true);
    const result = await triggerRefetch(source, source ? refetchable : Math.min(refetchable, 500));
    setPending(false);
    if (!result.success) {
      onError(result.message || "Failed to start refetch.");
      return;
    }
    onTriggered();
  };
  ...
  return (
    <button
      ...
      title={disabled ? "Another refetch job is already running" : undefined}
      className={cn(
    ...
    >
      ...
      {pending ? "Starting..." : source ? "Refetch" : "Refetch All"}
    </button>
  );
}

export function RefetchArticlesPanel({ rows }: { rows: SourceRefetchRow[] }) {
  ...
  const totalRefetchable = useMemo(() => rows.reduce((sum, r) => sum + (Number(r.refetchable) || 0), 0), [rows]);
  const sortedRows = useMemo(
    () => [...rows].filter((r) => r.refetchable > 0).sort((a, b) => b.refetchable - a.refetchable),
    [rows],
  );

  const poll = async () => {
    const s = await fetchRefetchStatus();
    if (!s) return;
    ...
    setStatus(s);
    if (!didInitialRefresh.current && totalRefetchable > 0) {
  ...
  async function handleStop() {
    ...
    try {
      await fetch("/api/v1/admin/commands/stop", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "refetch-failed" }),
      });
      poll();
    } catch { /* ignore */ }
    setStopping(false);
    ...
    },
    {
      key: "action", label: "", width: "140px", render: (_v, row) => (
        <RefetchButton
          ...
          refetchable={Number(row.refetchable) || 0}
          disabled={isRunning}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-100 bg-amber-50/60 p-4 dark:border-amber-900/40 dark:bg-amber-900/10">
        <div>
          <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">
            {totalRefetchable.toLocaleString()} articles ready to refetch
          </div>
          <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
            Failed articles with no cached text — re-fetches the page first, then re-extracts.
          </p>
        </div>
        <RefetchButton
          source={null}
          refetchable={totalRefetchable}
          disabled={isRunning}
          onTriggered={handleTriggered}
          onError={handleError}
        />
      ...
      {isRunning && current && (
        <div className="flex items-center justify-between gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
          <div className="flex items-center gap-3">
            ...
            <span>
              Refetching {current.source ?? "all sources"}: {current.retried}/{current.total}
